...
2. נפתח חלון OAuth בדפדפן והלקוח מאשר גישה
3. **החלון לא חוזר לבד** — ה-redirect של שני הספקים מצביע על localhost:
   - **Claude:** אחרי האישור claude.ai מציג **קוד להעתקה** — מעתיקים,
     חוזרים לטאב של האשף ומדביקים בתיבת "הדבק כאן"
   - **ChatGPT:** אחרי האישור הדפדפן ינסה לפתוח `localhost:1455` וייכשל —
     מעתיקים את **הכתובת המלאה** משורת הכתובת ומדביקים באותה תיבה

לאחר ההתחברות, המודל נקבע אוטומטית לברירת מחדל תקינה של הספק
(למשל `openai-codex/gpt-5.3-codex` — מודלי spark לא זמינים לחשבונות
ChatGPT, והקוד יודע להימנע מהם).
