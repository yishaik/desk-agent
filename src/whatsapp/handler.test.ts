...
describe('handleMessage — self-chat gate integration', () => {
  ...
  it('DOES process self-chat via phone JID when LID is missing (does not silent-return)', async () => {
    ...
    mockGetSelfChatJid.mockReturnValue(null);
  ...
  it('still skips fromMe to another person when LID is missing (fromMe is not auth)', async () => {
    mockGetOwnerLid.mockReturnValue(null);
    mockGetSelfChatJid.mockReturnValue('1234567890:123@s.whatsapp.net');
  ...
  it('skips when there is no self-chat JID at all (no inbound self JID and no fallback)', async () => {
    mockGetOwnerLid.mockReturnValue(null);
    mockGetSelfChatJid.mockReturnValue(null);
